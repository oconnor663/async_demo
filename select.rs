...
static SELECT_FDS: Mutex<Vec<libc::c_int>> = Mutex::new(Vec::new());
...
async fn accept(listener: &mut TcpListener) -> anyhow::Result<(TcpStream, SocketAddr)> {
    futures::future::poll_fn(|context| match listener.accept() {
        ...
        Err(e) if e.kind() == WouldBlock => {
            ...
            SELECT_FDS.lock().unwrap().push(listener.as_raw_fd());
            Poll::Pending
        }
...
async fn read(stream: &mut TcpStream, buf: &mut [u8]) -> anyhow::Result<usize> {
    futures::future::poll_fn(|context| match stream.read(buf) {
        ...
        Err(e) if e.kind() == WouldBlock => {
            WAKERS.lock().unwrap().push(context.waker().clone());
            SELECT_FDS.lock().unwrap().push(stream.as_raw_fd());
            Poll::Pending
        }
        Err(e) => Poll::Ready(Err(e.into())),
    })
...
fn main() -> anyhow::Result<()> {
    ...
    loop {
        ...
        // Block on the SELECT_FDS until something is readable.
        unsafe {
            let mut fd_set = MaybeUninit::<libc::fd_set>::uninit();
            libc::FD_ZERO(fd_set.as_mut_ptr());
            let mut fd_max = 0;
            for fd in SELECT_FDS.lock().unwrap().drain(..) {
